...
# 上传单个发票
def upload_invoice(client, file_path, headers):
    try:
...
# 获取发票详情
def get_invoice_details(client, invoice_id, headers):
    try:
        response = client.get(
...
def main():
    ...
    with httpx.Client() as client:
        ...
        for idx, file_path in enumerate(pdf_files, 1):
            ...
            success, result = upload_invoice(client, file_path, headers)
            ...
            if success:
                ...
                # 获取发票详情
                if invoice_id:
                    invoice_details = get_invoice_details(client, invoice_id, headers)
                    if invoice_details:
