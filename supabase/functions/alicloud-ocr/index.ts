...
class AliCloudOCRClient {
  ...
  accessKeySecret;
  endpoint;
  constructor(accessKeyId, accessKeySecret, region = 'cn-hangzhou'){
  ...
  /**
   * 发票OCR识别 (支持图片和PDF)
   */ async recognizeInvoice(fileUrl) {
    const params = {
      Url: fileUrl
    };
  ...
  /**
   * Base64图片/PDF发票OCR
   */ async recognizeInvoiceFromBase64(base64Data, isPdf = false) {
    const params = {
      body: base64Data
    };
    // PDF文件需要指定页码
  ...
  /**
   * 火车票OCR识别
   */ async recognizeTrainTicket(fileUrl) {
  ...
  /**
   * Base64火车票OCR
   */ async recognizeTrainTicketFromBase64(base64Data) {
    return this.callOCRAPI('RecognizeTrainInvoice', {
      body: base64Data
    });
  }
  /**
   * 通用OCR识别
   */ async recognizeGeneral(imageUrl) {
    return this.callOCRAPI('RecognizeGeneral', {
      Url: imageUrl
    });
  }
  async callOCRAPI(action, params) {
    try {
      // 生成时间戳和随机数 - 严格按照阿里云格式
      ...
      const requestBody = sortedKeys.map((key)=>{
   ...
   */ async generateSignature(method, params) {
...
function parseInvoiceData(ocrData) {
  // 解析返回的JSON字符串
  let data = ocrData;
...
function parseTrainTicketData(ocrData) {
  // 解析返回的JSON字符串
  let data = ocrData;
  if (typeof ocrData === 'string') {
    try {
...
serve(async (req)=>{
  ...
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      const action = formData.get('action') || 'invoice';
      if (file) {
        // 将文件转换为Base64
