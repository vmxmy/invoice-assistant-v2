...
class IconAdapter {
  ...
  static Map<String, dynamic> get mappingStats => {
    ...
    'categories': {
      'documents': _iconMapping.keys.where((k) => k.contains('doc') || k.contains('text') || k.contains('article')).length,
      'actions': _iconMapping.keys.where((k) => k.contains('edit') || k.contains('delete') || k.contains('add')).length,
      'navigation': _iconMapping.keys.where((k) => k.contains('arrow') || k.contains('chevron') || k.contains('back')).length,
      'status': _iconMapping.keys.where((k) => k.contains('check') || k.contains('error') || k.contains('warning')).length,
    }
  };
}
