...
class GetInvoiceDetailUseCaseProduction {
  ...
  Future<InvoiceEntity> call(String invoiceId) async {
    ...
    try {
      ...
      stopwatch.stop();
      _logPerformance(invoiceId, stopwatch.elapsedMilliseconds,
  ...
  Duration _calculateDelay(int attempt) {
    final delay = Duration(
      milliseconds:
          (_baseDelay.inMilliseconds * (_backoffMultiplier * attempt)).round(),
    );
  ...
  /// 性能日志
  void _logPerformance(String invoiceId, int milliseconds,
      {required bool success, dynamic error}) {
    final status = success ? '✅' : '❌';
    final errorInfo = error != null ? ' Error: $error' : '';
    AppLogger.debug(
        '📊 [GetInvoiceDetail] $status Invoice: $invoiceId, Time: ${milliseconds}ms$errorInfo',
        tag: 'Debug');
  }
  ...
  void _logRetry(String invoiceId, int attempt, Duration delay, dynamic error) {
    AppLogger.debug(
        '🔄 [GetInvoiceDetail] Retry $attempt for $invoiceId after ${delay.inMilliseconds}ms. Error: $error',
        tag: 'Debug');
  }
}
