...
class OptimizedReimbursementSetCard extends StatelessWidget {
  ...
  Widget _buildDateRangeRow(BuildContext context) {
    ...
    final dateRangeText =
        reimbursementSet.dateRangeText ?? reimbursementSet.smartDateRangeText;
  ...
  Future<void> _exportReimbursementSetPdfs(BuildContext context) async {
    try {
      ...
      final confirmed = await UnifiedBottomSheet.showConfirmDialog(
        ...
        title: '导出报销集',
        content:
            '确定要导出 "${reimbursementSet.setName}" 中的全部发票PDF吗？\n\n共包含 ${reimbursementSet.invoiceCount} 张发票。',
        confirmText: '导出',
      ...
      if (invoicesData.isEmpty) {
        ...
        if (context.mounted) {
          CupertinoNotificationUtils.showError(context, '报销集中没有可导出的发票');
        }
        return;
      }
      ...
      // 生成发票信息CSV文件并添加到压缩包
      final csvContent = _generateInvoiceListCsv(
          invoicesData, successCount, noFileCount, downloadFailCount);
      ...
      if (Platform.isIOS) {
        // iOS平台：直接分享压缩包
        await _shareReimbursementSetZip(
            Uint8List.fromList(zipData), zipFileName, successCount);
        if (context.mounted) {
          CupertinoNotificationUtils.showSuccess(context,
      ...
      if (context.mounted) {
        ...
        CupertinoNotificationUtils.showError(
            context, '导出失败：${InvoiceFileUtils.getDownloadErrorMessage(e)}');
      }
    }
  }
  ...
  String _generateReimbursementSetZipFileName() {
    // 清理文件名中的特殊字符
    final cleanSetName =
        reimbursementSet.setName.replaceAll(RegExp(r'[<>:"/\\|?*]'), '_');
  ...
  /// iOS平台分享压缩包
  Future<void> _shareReimbursementSetZip(
      Uint8List zipData, String fileName, int successCount) async {
    try {
  ...
  Future<String?> _saveReimbursementSetZip(
      Uint8List zipData, String fileName, int successCount) async {
    try {
      final outputFile = await FileOperationService.saveFile(
  ...
  String _generateInvoiceListCsv(List<dynamic> invoices, int successCount,
      int noFileCount, int downloadFailCount) {
    final buffer = StringBuffer();

    // CSV表头（中文列名，便于用户理解）
    buffer.writeln(
        '发票号码,开票日期,消费日期,销售方名称,购买方名称,发票金额,税额,币种,类别,状态,发票类型,是否验证,数据来源,创建时间,文件大小KB');
