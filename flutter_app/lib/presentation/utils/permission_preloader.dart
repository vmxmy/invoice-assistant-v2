...
class PermissionPreloader {
  ...
  Future<void> preloadPermissions({
    ...
    try {
      ...
      if (user == null || user.emailConfirmedAt == null) {
        AppLogger.debug('🔐 [PermissionPreloader] 用户未认证，跳过权限预加载',
            tag: 'Permission');
        return;
      }
      ...
      if (hasValidCache) {
        AppLogger.debug('🔐 [PermissionPreloader] 发现有效缓存，触发BLoC加载',
            tag: 'Permission');
        // 有缓存时，触发BLoC加载（会自动使用缓存）
        permissionBloc.add(const LoadPermissions());
      } else {
        AppLogger.debug('🔐 [PermissionPreloader] 无有效缓存，后台预加载权限',
            tag: 'Permission');
        // 无缓存时，在后台预加载权限但不阻塞UI
  ...
  void _backgroundPreload(PermissionBloc permissionBloc) {
    Future.delayed(const Duration(milliseconds: 100), () async {
      try {
        AppLogger.debug('🔐 [PermissionPreloader] 执行后台权限预加载',
            tag: 'Permission');
        permissionBloc.add(const LoadPermissions());
      } catch (e) {
        AppLogger.error(
  ...
  Future<void> onAuthStateChanged({
    ...
    try {
      AppLogger.debug('🔐 [PermissionPreloader] 认证状态变更: ${authState.event}',
          tag: 'Permission');
      ...
      switch (authState.event) {
        ...
        case AuthChangeEvent.tokenRefreshed:
          ...
          AppLogger.debug('🔐 [PermissionPreloader] Token刷新，更新权限',
              tag: 'Permission');
  ...
  Future<void> _handleUserLogin({
    ...
    if (currentUserId == null || currentUser?.emailConfirmedAt == null) {
      AppLogger.warning('🚨 [PermissionPreloader] 用户登录但未验证邮箱',
          tag: 'Permission');
      return;
    }
    ...
    AppLogger.info('✅ [PermissionPreloader] 用户登录处理完成: $currentUserId',
        tag: 'Permission');
  }
  ...
  Future<void> _handleUserLogout({
    ...
    AppLogger.info('👋 [PermissionPreloader] 用户登出处理: $logoutUserId',
        tag: 'Permission');
  ...
  Future<bool> smartPermissionCheck({
    ...
    try {
      ...
      if (currentState is PermissionInitial ||
          currentState is PermissionEmpty) {
        // 权限未加载，触发加载
        AppLogger.debug('🔐 [PermissionPreloader] 权限未加载，触发预加载',
            tag: 'Permission');
        permissionBloc.add(const LoadPermissions());
  ...
  Future<void> _waitForPermissionLoad(
    ...
    while (DateTime.now().difference(startTime) < timeout) {
      ...
      if (state is PermissionLoaded ||
          state is PermissionError ||
          state is PermissionEmpty) {
        break;
      }
