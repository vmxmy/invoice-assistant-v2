...
class ReimbursementSetOperationUtils {
  ...
  static String generateSmartName(List<InvoiceEntity> invoices) {
    ...
    final totalAmount = invoices.fold<double>(
        0.0, (sum, invoice) => sum + invoice.displayAmount);
  ...
  static String _getConsumptionDateRange(List<InvoiceEntity> invoices) {
    ...
    final startDate =
        '${earliestDate.year}-${earliestDate.month.toString().padLeft(2, '0')}-${earliestDate.day.toString().padLeft(2, '0')}';
    final endDate =
  ...
  static Map<String, dynamic> getInvoiceStatistics(
    ...
    for (final invoice in invoices) {
      ...
      monthlyStats[monthKey]!['amount'] =
          monthlyStats[monthKey]!['amount'] + invoice.displayAmount;
  ...
  static Widget _buildStatusSelectionContent({
    ...
    return Column(
      ...
      children: [
        ...
        // 状态选项列表
        ...ReimbursementSetStatus.values.map(
  ...
  static Widget _buildStatusOption({
    ...
    return Container(
      ...
      child: Semantics(
        ...
        hint: isCurrentStatus ? '当前状态：$statusText' : '点击切换到$statusText状态',
        child: CupertinoButton(
  ...
  static Color _getContrastColor(
    ...
    // 计算与白色和黑色的对比度
    final whiteContrast =
        _calculateContrastRatio(backgroundColor, Colors.white);
    final blackContrast =
        _calculateContrastRatio(backgroundColor, Colors.black);
    final onSurfaceContrast =
        _calculateContrastRatio(backgroundColor, colorScheme.onSurface);
    final surfaceContrast =
        _calculateContrastRatio(backgroundColor, colorScheme.surface);
