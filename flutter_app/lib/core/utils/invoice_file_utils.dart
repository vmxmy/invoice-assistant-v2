...
class InvoiceFileUtils {
  ...
  static Future<Uint8List> _downloadWithRetry({
    ...
    try {
      _logSecure('📥 [下载] 尝试下载 (${retryCount + 1}/${_maxRetries + 1}): $invoiceNumber');
      ...
      final Uint8List fileBytes = await SupabaseClientManager.downloadFile(
        ...
        throw createSafeException('下载超时', 'Download timeout after ${_downloadTimeout.inSeconds} seconds');
      });
      
      // 文件大小安全检查
      if (!_isFileSizeSafe(fileBytes.length)) {
        _logSecure('❌ [安全] 文件大小超限: ${fileBytes.length} bytes');
        throw createSafeException('文件过大', 'File size ${fileBytes.length} exceeds limit');
      }
  ...
  /// 下载发票PDF到临时文件
  /// 返回File对象，用于分享或打开
  static Future<File> downloadInvoicePdfToTempFile(InvoiceEntity invoice) async {
    try {
      // 获取PDF字节数据
  ...
  static Future<String> getPdfDownloadUrl(InvoiceEntity invoice) async {
    ...
    try {
      // 提取文件路径
      final filePath = SupabaseClientManager.extractFilePathFromUrl(invoice.fileUrl!);
  ...
  static String getDownloadErrorMessage(dynamic error) {
    ...
    // 认证相关错误
    if (errorString.contains('401') || errorString.contains('未认证') || 
        errorString.contains('unauthenticated')) {
      return '请先登录';
    }
    
    // URL解析错误
    if (errorString.contains('无法解析文件路径') || errorString.contains('invalid url') ||
        errorString.contains('url格式') || errorString.contains('不受信任的域名')) {
      return '文件链接格式错误';
    }
    ...
    // 文件大小或格式错误
    if (errorString.contains('file size') || errorString.contains('format') ||
        errorString.contains('文件大小') || errorString.contains('格式')) {
      return '文件格式或大小异常';
    }
    ...
    if (errorString.contains('500') || errorString.contains('502') || 
        errorString.contains('503') || errorString.contains('server error')) {
  ...
  static Exception createSafeException(String userMessage, dynamic originalError) {
    // 在调试模式下记录详细错误
    if (AppConfig.enableLogging && AppConfig.isDebugMode) {
  ...
  static bool _isValidPdfContent(Uint8List fileBytes) {
    ...
    // 检查PDF文件尾部是否有EOF标记
    if (fileBytes.length >= 5) {
      final String tail = String.fromCharCodes(fileBytes.skip(fileBytes.length - 5));
      if (!tail.contains('%%EOF')) {
        // 有些PDF文件可能在EOF后还有额外字符，检查更大的尾部范围
        if (fileBytes.length >= 20) {
          final String largeTail = String.fromCharCodes(fileBytes.skip(fileBytes.length - 20));
