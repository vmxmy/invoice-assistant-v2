...
class CacheIsolationValidator {
  static final CacheIsolationValidator _instance = CacheIsolationValidator._internal();
  ...
  Future<Map<String, dynamic>> _validatePermissionCacheIsolation() async {
    ...
    return {
      ...
      'validation': {
        ...
        'appropriateExpiration': cacheStats['isExpired'] == false || cacheStats['isExpired'] == null,
      }
    };
  }
  ...
  /// 生成整体评估
  Map<String, dynamic> _generateOverallAssessment(Map<String, dynamic> results) {
    final issues = <String>[];
    ...
    final invoiceValidation = invoiceCache['validation'] as Map<String, dynamic>? ?? {};
    ...
    // 检查权限缓存
    final permissionCache = results['permissionCache'] as Map<String, dynamic>? ?? {};
    final permissionValidation = permissionCache['validation'] as Map<String, dynamic>? ?? {};
    ...
    final score = totalChecks > 0 ? (successes.length / totalChecks * 100).round() : 0;
  ...
  Future<void> performHealthCheck() async {
    ...
    try {
      ...
      final assessment = results['overallAssessment'] as Map<String, dynamic>? ?? {};
      ...
      if (issues.isNotEmpty) {
        AppLogger.info('💡 [CacheValidator] 建议: ${assessment['recommendation']}', tag: 'Validation');
      }
