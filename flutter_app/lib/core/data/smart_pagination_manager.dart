...
class SmartPaginationManager {
  static final SmartPaginationManager _instance = SmartPaginationManager._internal();
  ...
  Future<PaginationResult<InvoiceEntity>> getInvoicesPage({
    ...
    required Future<List<InvoiceEntity>> Function(int page, int pageSize) fetchFunction,
    Map<String, dynamic>? filters,
  }) async {
    final pageKey = _generatePageKey(page, pageSize, filters);
    
    // 1. 检查缓存
    if (_cachedPages.containsKey(pageKey) && !_cachedPages[pageKey]!.isExpired) {
  ...
  Map<String, dynamic> getCacheStats() {
    return {
      ...
      'expiredPages': _cachedPages.values
          .where((page) => page.isExpired)
          .length,
    };
  }
